...
The output of this script will be a table in markdown format, which you can paste into the README.md file of the repository.
"""
...
def generate_library_table(filename):
    ...
    with open(filename, "w") as f:
        ...
        for k, v in LIBRARIES.items():
            ...
            license = f"[{v['license_name']}]({v['license_link']})" # link to the license
            # if build_info and build_info.tarball_name and build_info.download_link is avaliabl then set the download_source
            download_source = ""
            if v.get('build_info') and v['build_info'].get('tarball_name') and v['build_info'].get('download_link'):
...
def generate_build_manifest(output_file):
    ...
    for library_name, library_info in LIBRARIES.items():
        ...
        if not all([download_url, build_dir, tarball_name]):
            print(f"Warning: Missing information for {library_name} in build manifest generation.")
            continue
...
def main():
    ...
    parser.add_argument("--mode", choices=["both", "table", "manifest"], default="both",
                        help="Choose the mode: 'both' for both files, 'table' for library table, 'manifest' for build manifest")
    parser.add_argument("--output", type=str, default="generated_versions_table.md",
    ...
    if args.mode == "both":
        library_table = generate_library_table(args.output)
